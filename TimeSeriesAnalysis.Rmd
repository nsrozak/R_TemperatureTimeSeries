...
Training data is comprised of the first 51 years, the last 20 years are used to test the models. 
...
The classical decomposition of a time series is $X_t=m_t+x_t+S_t$ where $m_t$ is the trend, $s_t$ is the seasonal component, and $S_t$ is the stationary process.
...
The confidence interval for $\hat\lambda$ is $\ell(\lambda)\geq\ell(\hat\lambda)-\frac{1}{2}\chi^2_{1,1-\alpha}$.
...
I created lagged variables based off of significant lags from the autocorrelation plot. These variables are used as predictors for the machine learning models.

```{r create data frame for machine learning}
# create data frame
data_ml <- monthly
...
```{r select columns for machine learning}
# select columns
...
The dataset is seasonal with consistent magnitudes, so I used the Holt-Winters seasonal method to build an additive model. However, I will not include a trend component in the model. The equation is $\hat{y}_{t+h}=\ell_t+s_{t+h-m(k+1)}$ where $k$ is the integer part of $\frac{h-1}{m}$ and $m$ is the season. The level pattern is $\ell_t=\alpha(y_t-s_{t-m})+(1-\alpha)\ell_{t-1}$ and the season is $s_t=\gamma(y_t-\ell_{t-1})+(1-\gamma)s_{t-m}$. $\alpha$ and $\gamma$ are smoothing parameters.

```{r create holt winters model}
# create model
...
Akaike Information Criterion (AIC) is used to evaluate models based on their accuracy and complexity. The formula for AIC is $AIC=-2ln(L(\theta_q,\phi_p,\Theta_Q,\Phi_P,\sigma_z^2))+2k$ where $L(\theta_q,\phi_p,\Theta_Q,\Phi_P,\sigma_z^2)$ are the  maximum likelihood estimators and $k$ is the number of estimated parameters. 

```{r output exponential smoothing AIC}
# output AIC
...
The exponential smoothing model arguably passes the diagnostics test that the residuals resemble white noise, and it does not pass the diagnostics test that the residuals come from a normal distribution. Therefore, this model can be improved and its forecasts might not perform well. 

#### Residuals Resemble White Noise

***Time Series Plot***

```{r exponential smoothing residuals time series plot, message=FALSE,fig.align='center',fig.width=6,fig.height=4}
# obtain residuals
...
The residuals mostly resemble white noise for the exponential smoothing model.

***ACF and PACF Plots***

```{r exponential smoothing residuals acf plot,fig.align='center',fig.width=6,fig.height=4}
# create acf data frame
...
The first lag of the residuals lies outside the 95% confidence interval containing 0. Since 20 lags are plotted, it is reasonable for at most one of the lags to lie outside the confidence interval. 

```{r exponential smoothing residuals pacf plot,fig.align='center',fig.width=6,fig.height=4}
# create acf data frame
...
```{r exponential smoothing qq plot,message=FALSE,fig.align='center',fig.width=6,fig.height=4}
# qq plot
...
```{r exponential smoothing shapiro-wilk test}
# shapiro-wilk test
...
```{r function for outputting coefficient confidence intervals}
...
```{r initial coefficient confidence intervals}
...
The AR2 and SAR1coefficient confidence intervals contain 0, so I will remove them from the model.

### Final Model

```{r sarima final model}
# create final model
...
```{r sarima final model coefficients}
# variance matrix for coefficients
...
The final model is SARIMA$(3,0,0)\times(2,1,1)_{12}$ with $\phi(B)\Phi(B^{12})(1-B^{12})X_t=\Theta(B^{12})Z_t$ with $\phi(B)=1-0.223B-0.139B^3$, $\Phi(B^{12})=1+0.097B^{24}$, and $\Theta(B^{12})=1-0.891B^{12}$. The solved equation is $X_t=0.223X_{t-1}+0.139X_{t-3}+X_{t-12}-0.223X_{t-13}-0.139X_{t-15}-0.097X_{t-24}+0.021X_{t-25}+0.013X_{t-27}+0.097X_{t-36}-0.021X_{t-37}-0.013X_{t-39}+Z_t-0.891Z_{t-12}$.

```{r output sarima final model AIC}
...
```{r sarima residuals time series plot,message=FALSE, fig.align='center',fig.width=6,fig.height=4}
# residuals
...
```{r sarima residuals acf plot,fig.align='center',fig.width=6,fig.height=4}
# create acf data frame
...
All residuals are within the 95% confidence interval containing 0, so they appear to be uncorrelated.

```{r sarima residuals pacf plot,fig.align='center',fig.width=6,fig.height=4}
# create acf data frame
...
```{r sarima portmanteau statistics}
...
```{r sarima qq plot, message=FALSE,fig.align='center',fig.width=6,fig.height=4}
# qq plot
...
I used Forward Chaining Cross Validation. The first fifteen years are used to train the first fold's model. At each iteration, three additional years are added to the train dataset and the model gets tested on the next three years. 
...
Models predict differenced time series values, $\nabla_{12}Y_t$. The formula for converting the time series back to the original units, $T_t$, is $T_t=(\nabla_{12}Y_t+Y_{t-12})^2$.

```{r converting back to original units}
...
```{r random forest cross validatation}
# function for cross validation
rf_cv <- function(tree){
  rmse_lst <- NULL
  for(i in 1:length(train_folds)){
...
I used forward chaining cross validation to tune the `interaction.depth` and `shrinkage` parameters. 

```{r gradient boosted cross validatation,message=FALSE}
# function for cross validation
...
```{r gradient boosted final model,message=FALSE}
# create gbt model
set.seed(347)
...
```{r linear regression final model}
...
```{r ridge regression cross validatation}
# function for cross validation
...
```{r ridge regression final model}
# create linear regression model
...
```{r lasso regression cross validatation}
# store rmse
lasso_rmse <- NULL
# cross validation
for (lambda in lambdas){
...
```{r lasso regression final model}
# create linear regression model
...
I used forward chaining cross validation to tune the `cost`, `epsilon`, and the `kernel`. Also, I tuned specific parameters for each kernel: `gamma` for radial basis, sigmoid, and polynomial; `coef0` for sigmoid and polynomial; and `degree` for polynomial. The code below takes a while to run, so I commented it out when making this report.
...
```{r neural net cross validatation}
# function for cross validation
...
```{r output rmse from all of the models}
...
The model with the lowest RMSE for the next 20 years is SARIMA, and the model with the lowest RMSE for the next year is exponential smoothing. 

```{r function for plotting predictions}
...
```{r exponential smoothing time series plot,fig.align='center',fig.width=10,fig.height=4}
...
```{r exponential smoothing prediction plot,fig.align='center',fig.width=10,fig.height=4}
...
```{r sarima time series plot,fig.align='center',fig.width=10,fig.height=4}
# obtain confidence interval
...
```{r sarima prediction plot,fig.align='center',fig.width=10,fig.height=4}
